    ...
    for i, x in enumerate(test_loader):
        with torch.no_grad():  # Disable gradient calculations
            if len(x) > 1:
                ...
                pred_distance_transform, pred_peak_locations = infer(model, x, downsampling_factor=1)

            if args.evaluate:
                assert len(x) > 1
                # Using scipy's find_peaks for actual peak detection
                actual_peak_locations = find_peaks(x[1].cpu().numpy().flatten())[0]
